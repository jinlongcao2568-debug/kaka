...
- YGP 口径：YGP 可作为 `data.ggzy` 原文链接指向时的 P13B 原文 readback 能力，以及后续广东其他城市独立适配的基础能力；但不恢复为广州主采集源，广州主源仍是广州交易集团。YGP `url-mapping` 抓到 SPA 壳页时，应通过公开详情接口发现或只读浏览器网络回放取得原文详情；阻断、未命中或字段缺失只能写 taxonomy，不能写排除结论。
