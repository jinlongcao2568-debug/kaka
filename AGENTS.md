...
- YGP 口径：YGP 可作为 `data.ggzy` 原文链接指向时的 P13B 原文 readback 能力，以及后续广东其他城市独立适配的基础能力；但不恢复为广州主采集源，广州主源仍是广州交易集团。YGP `url-mapping` 抓到 SPA 壳页时，应通过公开详情接口发现或只读浏览器网络回放取得原文详情；阻断、未命中或字段缺失只能写 taxonomy，不能写排除结论。YGP 01-12 能力应实现为“项目流程矩阵”：输入 YGP 项目/原文 URL 后解析项目参数，读取 `nodeList`，再逐节点读取 `detail`，输出流程是否存在、详情 URL、发布日期、有限字段 probe 和 hash；它服务于 P13B 原文回溯和广东其他城市复用，不是核验结论逻辑。广东源分工固定为：广州继续用广州交易集团独立适配，深圳使用深圳公共资源交易中心独立适配，其他广东城市优先以 YGP/粤公平矩阵作为城市适配基础。
