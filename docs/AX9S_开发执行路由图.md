...
- 当前 active packet 以 `control/current_task.yaml` 为准；本轮 active packet 为 `PTL-I100-140-real-public-verification-to-rule-evidence-pilot`，承接 139 已能生成的真实公开 Stage4 public verification carrier，验证其进入 Stage5 rule/evidence gate/readback；Stage5 rule/evidence readback 不得自动成为 Stage6 fact、客户可见材料或法律结论，不放开任意 crawler、登录/验证码/反爬绕过、public software release、未审批 live provider、真实下载、真实退款或自动退款。
...
- 本轮只做 `PTL-I100-140-real-public-verification-to-rule-evidence-pilot` 的真实 Stage4 public verification carrier 到 Stage5 rule/evidence gate/readback 受控验证与对应控制面/测试同步；不改 D1-D14 正文 / contracts / handoff / scripts / fixtures / Stage1/2/3/4/6/7/8/9 runtime / storage runtime / shared runtime，不把 Stage5 rule/evidence readback 升格为 Stage6 fact 或客户可见结论，不放开 external release、未审批 Stage8 live send、未审批 Stage9 live payment/delivery、真实下载、真实退款、自动退款或真实外部模型调用。
...
- 当前 active packet：`PTL-I100-140-real-public-verification-to-rule-evidence-pilot`；本轮把真实来源验证从“Stage4 public verification/readback pilot”推进到“Stage5 rule/evidence gate/readback pilot”，默认不执行任意 crawler、未审批 live provider 调用或真实客户下载。
- 当前候选池 next candidate：`PTL-I100-140-real-public-verification-to-rule-evidence-pilot` 已进入 active；完成后不自动激活下一包。
...
- `PTL-I100-140-real-public-verification-to-rule-evidence-pilot`：当前 active；把真实公开 Stage4 public verification carrier 送入 Stage5 rule/evidence gate/readback，验证 rule_hit/evidence/readback 绑定 verification run、source snapshot 和 parsed field refs，review/weak 结果 fail-closed 到 REVIEW，不生成 Stage6 fact 或客户可见输出。
