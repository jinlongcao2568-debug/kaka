...
- 当前 active packet 以 `control/current_task.yaml` 为准；本轮 active packet 为 `PTL-I100-133D-public-attachment-original-link-fetching`，只做真实公开附件原始链接抓取：覆盖 allowlisted 官方 PDF 原始 URL，直接做 bytes/hash/readback；不放开任意 crawler、登录/验证码/反爬绕过、public software release、未审批 live provider、真实下载、真实退款或自动退款。
...
- 本轮只做 `PTL-I100-133D-public-attachment-original-link-fetching` 的公开附件原始链接 fetch/readback 与测试；不改 D1-D14 正文 / contracts / handoff / scripts / fixtures / Stage1/3/4/5/6/7/8/9 runtime / storage runtime / shared runtime，不放开 external release、未审批 Stage8 live send、未审批 Stage9 live payment/delivery、真实下载、真实退款、自动退款或真实外部模型调用。
...
- 当前 active packet：`PTL-I100-133D-public-attachment-original-link-fetching`；本轮把真实来源验证从“入口页 fetch/readback”推进到“公开附件原始 URL 的二进制抓取与 fail-closed HTML 伪装下载分类”，默认不执行任意 crawler、未审批 live provider 调用或真实客户下载。
- 当前候选池 next candidate：`PTL-I100-133D-public-attachment-original-link-fetching` 已进入 active；完成后候选池应指向 134，但不自动激活。
...
- `PTL-I100-133D-public-attachment-original-link-fetching`：继续接官方公开附件原始链接，覆盖北京平台 PDF 原始 URL，区分“真实二进制附件成功抓取”和“HTML 伪装下载页 fail-closed”。
