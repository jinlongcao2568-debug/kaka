...
- 当前 active packet 以 `control/current_task.yaml` 为准；本轮 active packet 为 `PTL-I100-132-owner-operator-frontend-productization-workbench`，只做 owner/customer 前端产品化：把 Stage1-9 运营总览、证据链、销售闭环、客户交付、支付交付、provider/scheduler/audit/readback 与红线放到可观察工作台；不放开 public software release、未审批 live provider、真实下载、真实退款或自动退款。
...
- 本轮只做 `PTL-I100-132-owner-operator-frontend-productization-workbench` 的前端产品化工作台与测试；不改 D1-D14 正文 / contracts / handoff / scripts / fixtures / Stage1-9 runtime / storage runtime / shared runtime，不放开 external release、未审批 Stage8 live send、未审批 Stage9 live payment/delivery、真实下载、真实退款、自动退款或真实外部模型调用。
...
- 当前 active packet：`PTL-I100-132-owner-operator-frontend-productization-workbench`；本轮把现有 Owner Console / Customer Portal 产品化为可观察工作台，默认不执行 public release、未审批 live provider 调用或真实客户下载。
- 当前候选池 next candidate：`PTL-I100-132-owner-operator-frontend-productization-workbench` 已进入 active；完成后候选池应标记 completed，不自动激活新任务。
