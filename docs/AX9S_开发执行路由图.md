...
- 当前 active packet 以 `control/current_task.yaml` 为准；本轮 active packet 为 `PTL-I100-139-real-public-parser-to-verification-pilot`，承接 138 已能生成的真实公开 Stage3 parsed carrier，验证其进入 Stage4 public verification/readback；verification readback 不得自动成为 Stage5 rule hit、Stage6 fact 或客户可见结论，不放开任意 crawler、登录/验证码/反爬绕过、public software release、未审批 live provider、真实下载、真实退款或自动退款。
...
- 本轮只做 `PTL-I100-139-real-public-parser-to-verification-pilot` 的真实 Stage3 parsed carrier 到 Stage4 public verification/readback 受控验证与对应控制面/测试同步；不改 D1-D14 正文 / contracts / handoff / scripts / fixtures / Stage1/5/6/7/8/9 runtime / storage runtime / shared runtime，不把 Stage4 readback 升格为 Stage5 rule hit、Stage6 fact 或客户可见结论，不放开 external release、未审批 Stage8 live send、未审批 Stage9 live payment/delivery、真实下载、真实退款、自动退款或真实外部模型调用。
...
- 当前 active packet：`PTL-I100-139-real-public-parser-to-verification-pilot`；本轮把真实来源验证从“Stage3 parser/readback pilot”推进到“Stage4 public verification/readback pilot”，默认不执行任意 crawler、未审批 live provider 调用或真实客户下载。
- 当前候选池 next candidate：`PTL-I100-139-real-public-parser-to-verification-pilot` 已进入 active；完成后不自动激活下一包。
...
- `PTL-I100-139-real-public-parser-to-verification-pilot`：当前 active；把真实公开 Stage3 parsed carrier 送入 Stage4 public verification/readback，验证 parsed field refs、snapshot refs、evidence grade 和 fail-closed taxonomy，不生成 Stage5/Stage6/客户可见输出。
