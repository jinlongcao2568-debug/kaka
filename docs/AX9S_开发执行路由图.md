...
- 当前 active packet 以 `control/current_task.yaml` 为准；本轮 active packet 为 `PTL-I100-130-llm-assisted-parsing-review-and-sales-governance`，只做受治理 model-assist readback：字段抽取候选、证据摘要、复核 triage、销售话术草稿；不调用真实外部模型 provider，不把模型输出写成事实或客户结论。
...
- 本轮只做 `PTL-I100-130-llm-assisted-parsing-review-and-sales-governance` 的 model-assist governed readback；不改 D1-D14 正文 / contracts / handoff / scripts / fixtures，不放开 external release、未审批 Stage8 live send、未审批 Stage9 live payment/delivery、真实退款、自动退款或真实外部模型调用。
...
- 当前 active packet：`PTL-I100-130-llm-assisted-parsing-review-and-sales-governance`；本轮补受治理大模型辅助解析、复核、证据摘要和销售话术草稿，默认不执行真实外部模型调用。
- 当前候选池 next candidate：`PTL-I100-131-controlled-real-world-e2e-pilot-and-closeout`，状态 `CANDIDATE_NOT_ACTIVATED`；它只是候选提示，不自动激活。
