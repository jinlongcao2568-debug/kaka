...
- 当前 active packet 以 `control/current_task.yaml` 为准；当前 active packet 为 `PTL-GOV-107-mainline-candidate-shift-to-S34`；`PTL-S34-object-lineage-verification-handoff` 是 `current_mainline_next_candidate`，不是自动激活的当前执行包。
...
- `PTL-S34-object-lineage-verification-handoff`：current_mainline_next_candidate；这里只提示 Stage3-4 对象 lineage 与公开核验移交闭合的近端导航位置，不自动激活为当前执行包。
