...
- 当前 active packet 以 `control/current_task.yaml` 为准；本轮 active packet 为 `PTL-I100-134-owner-task-runner-real-source-ui`，只把 owner console 补成真实公开源受控执行面：页面列出 allowlisted 入口页/附件 profile，内部触发既有 Stage2 fetcher 并读回结果；不放开任意 crawler、登录/验证码/反爬绕过、public software release、未审批 live provider、真实下载、真实退款或自动退款。
...
- 本轮只做 `PTL-I100-134-owner-task-runner-real-source-ui` 的 owner console 真实来源执行面与测试；不改 D1-D14 正文 / contracts / handoff / scripts / fixtures / Stage1/3/4/5/6/7/8/9 runtime / storage runtime / shared runtime，不放开 external release、未审批 Stage8 live send、未审批 Stage9 live payment/delivery、真实下载、真实退款、自动退款或真实外部模型调用。
...
- 当前 active packet：`PTL-I100-134-owner-task-runner-real-source-ui`；本轮把真实来源验证从“已有 fetcher 能抓”推进到“owner console 可以直接受控执行入口页/附件抓取并回显读回”，默认不执行任意 crawler、未审批 live provider 调用或真实客户下载。
- 当前候选池 next candidate：`PTL-I100-134-owner-task-runner-real-source-ui` 已进入 active；完成后不自动激活下一包。
...
- `PTL-I100-134-owner-task-runner-real-source-ui`：把 owner console 接到 133A-133D 的 allowlisted real public fetcher，上线 internal-only profile catalog、capture 触发和 repository-backed readback，不引入新前端栈。
