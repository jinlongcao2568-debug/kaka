...
- 当前 active packet 以 `control/current_task.yaml` 为准；本轮 active packet 为 `PTL-I100-141-real-public-rule-evidence-to-stage6-product-package-pilot`，承接 140 已能生成的真实公开 Stage5 rule/evidence bundle，验证其进入 Stage6 product package readiness；Stage6 product package readiness 不得自动成为客户可见材料、法律结论或 Stage7/8/9 执行，不放开任意 crawler、登录/验证码/反爬绕过、public software release、未审批 live provider、真实下载、真实退款或自动退款。
...
- 本轮只做 `PTL-I100-141-real-public-rule-evidence-to-stage6-product-package-pilot` 的真实 Stage5 rule/evidence bundle 到 Stage6 product package readiness 受控验证与对应控制面/测试同步；不改 D1-D14 正文 / contracts / handoff / scripts / fixtures / Stage1/2/3/4/5/7/8/9 runtime / storage runtime / shared runtime，不把 Stage6 product package readiness 发布为客户可见材料、法律结论或下游执行，不放开 external release、未审批 Stage8 live send、未审批 Stage9 live payment/delivery、真实下载、真实退款、自动退款或真实外部模型调用。
...
- 当前 active packet：`PTL-I100-141-real-public-rule-evidence-to-stage6-product-package-pilot`；本轮把真实来源验证从“Stage5 rule/evidence gate/readback pilot”推进到“Stage6 product package readiness pilot”，默认不执行任意 crawler、未审批 live provider 调用或真实客户下载。
- 当前候选池 next candidate：`PTL-I100-141-real-public-rule-evidence-to-stage6-product-package-pilot` 已进入 active；完成后不自动激活下一包。
...
- `PTL-I100-141-real-public-rule-evidence-to-stage6-product-package-pilot`：当前 active；把真实公开 Stage5 rule/evidence bundle 送入 Stage6 product package readiness，验证产品包 carrier/readback 绑定 verification、snapshot、parse、rule/evidence gate refs，review/weak 结果 fail-closed 到 REVIEW_REQUIRED，不生成客户可见输出或下游执行。
