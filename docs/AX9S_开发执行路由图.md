...
- 当前 active packet 以 `control/current_task.yaml` 为准；当前 active packet 为 `PTL-GOV-108-mainline-candidate-shift-to-S45`；`PTL-S45-rule-evidence-dual-gate` 是 `current_mainline_next_candidate`，不是当前 active packet，也不是自动激活的当前执行包。
...
- `PTL-S45-rule-evidence-dual-gate`：current_mainline_next_candidate；这里只提示 Stage4-5 规则与证据双闸门闭合的近端导航位置，不自动激活为当前执行包。
