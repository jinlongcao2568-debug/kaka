...
- 当前 active packet 以 `control/current_task.yaml` 为准；本轮 active packet 为 `PTL-I100-133C-representative-local-platform-entry-fetchers`，只做代表性地方平台的官方 URL fetch/readback：覆盖北京市主平台、北京工程建设入口、北京经开区分平台、广东省 portal 等真实入口；不放开任意 crawler、登录/验证码/反爬绕过、public software release、未审批 live provider、真实下载、真实退款或自动退款。
...
- 本轮只做 `PTL-I100-133C-representative-local-platform-entry-fetchers` 的地方平台入口 fetch/readback 与测试；不改 D1-D14 正文 / contracts / handoff / scripts / fixtures / Stage1/3/4/5/6/7/8/9 runtime / storage runtime / shared runtime，不放开 external release、未审批 Stage8 live send、未审批 Stage9 live payment/delivery、真实下载、真实退款、自动退款或真实外部模型调用。
...
- 当前 active packet：`PTL-I100-133C-representative-local-platform-entry-fetchers`；本轮把真实来源验证从“国家级核验入口”推进到“代表性地方平台入口的 success-path 与 raw-shell fail-closed 分类”，默认不执行任意 crawler、未审批 live provider 调用或真实客户下载。
- 当前候选池 next candidate：`PTL-I100-133C-representative-local-platform-entry-fetchers` 已进入 active；完成后候选池应指向 133D 或 134，但不自动激活。
...
- `PTL-I100-133C-representative-local-platform-entry-fetchers`：继续接代表性地方平台入口，覆盖北京市主平台、北京工程建设入口、北京经开区分平台和广东省 portal，区分“直接 HTML 成功入口”和“200 但只返回前端壳页的 portal”。
