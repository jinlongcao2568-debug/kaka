...
- 本轮只做 `PTL-I100-143G-public-web-capture-doc-sync-and-order-review` 的 D 文档补表、路线图、控制面和测试同步；不改 D1-D14 正文语义 / contracts / handoff / scripts / fixtures / Stage1-9 runtime / storage runtime / shared runtime，不把商业钩子线索直接变客户可见证据包，不放开 external release、未审批 live provider、自动解验证码、真实下载、真实退款、自动退款或真实外部模型调用。
