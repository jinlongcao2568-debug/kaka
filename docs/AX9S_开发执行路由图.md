...
- 当前 active packet 以 `control/current_task.yaml` 为准；本轮 active packet 为 `PTL-I100-137-degraded-real-public-site-hardening`，承接 136 中仍 degraded/fail-closed 的 8 个真实公开站点 profile：能合规走公开静态/轻量入口的升级为 snapshot/readback，不能合规读取正文的继续稳定 fail-closed taxonomy；不放开任意 crawler、登录/验证码/反爬绕过、public software release、未审批 live provider、真实下载、真实退款或自动退款。
...
- 本轮只做 `PTL-I100-137-degraded-real-public-site-hardening` 的 Stage2 degraded real public site hardening 与对应控制面/测试同步；不改 D1-D14 正文 / contracts / handoff / scripts / fixtures / Stage1/3/4/5/6/7/8/9 runtime / storage runtime / shared runtime，不放开 external release、未审批 Stage8 live send、未审批 Stage9 live payment/delivery、真实下载、真实退款、自动退款或真实外部模型调用。
...
- 当前 active packet：`PTL-I100-137-degraded-real-public-site-hardening`；本轮把真实来源验证从“批量 URL fetcher 硬化”推进到“对 8 个 degraded profile 逐一站点级合规处置”，默认不执行任意 crawler、未审批 live provider 调用或真实客户下载。
- 当前候选池 next candidate：`PTL-I100-137-degraded-real-public-site-hardening` 已进入 active；完成后不自动激活下一包。
...
- `PTL-I100-137-degraded-real-public-site-hardening`：当前 active；对 136 中 JZSC、CreditChina、GSXT、广东省/云浮等 8 个 degraded profile 做站点级公开路径硬化或稳定 fail-closed 分类。
