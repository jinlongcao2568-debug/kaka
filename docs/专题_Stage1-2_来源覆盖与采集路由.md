...
本专题是 `docs/AX9S_Stage1-9_执行矩阵与子漏斗.md` 在 **Stage1-2** 维度的展开文档：

- `AX9S_Stage1-9_执行矩阵与子漏斗.md` 负责主链、验收、PASS/REVIEW/BLOCK 与补证回退
- 本专题只展开 Stage1-2 的 source authority、route semantics、collection taxonomy 和 readback 输入边界
...
- `docs/AX9S_Stage1-9_执行矩阵与子漏斗.md`：本专题只承接其中的来源覆盖、采集路由、状态机、早期线索和字段血缘输入，不承接完整能力矩阵和 Stage4-5 规则定义。
- `control/stage16_file_analysis_task_contract.yaml`：保留更细的 Stage1-6 机器批次、准入标准和 anti-drift 规则，不替代本专题的人类可读说明。

### 2.1 与执行矩阵 / Stage1-6 机器契约的分工

本专题是来源和路由规范，不是风控规则目录。与执行矩阵 / Stage1-6 机器契约的分工如下：
...
### 13.8 规则经验长稿的 Stage1-2 消费入口

archive 长稿中属于 Stage1-2 的资料，进入本专题时只作为候选发现、watchlist、source intelligence 和来源质量反馈，不进入 Stage4-5 主规则。

| 原稿章节 | 应消费资料 | Stage1-2 落点 | 边界 |
|---|---|---|---|
| `2.1 公告前机会层` | 政府采购意向、招标计划、招标文件提前公示、采购意向到公告间隔 | `pre_notice_signal`, `tender_plan_watchlist`, `clock_chain_profile` | 不写成候选后证据包 |
| `4.6 项目来源库、AI采集与项目档案` | 专业招标网站、政府采购网、公共资源平台、重大项目清单、主管部门平台、土地招拍挂、地方新闻、设计院/咨询公司中标、行业官网、上下游反推、圈子弱线索 | source registry / source intelligence / project folder | 无公开回链只作弱线索 |
| `8.1 前置分流与公告前机会` | `pre_notice_type`, `source_channel_type`, `source_lead_time_band`, `project_lifecycle_stage`, `source_quality_score`, `source_update_latency` | Stage1 source profile / Stage2 route profile | 候选字段，不替代正式 schema |
| `8.3 项目筛选与盲投运营` 前半 | 项目筛选、盲投漏斗、团队产能、跟进记录 | internal watchlist priority | 不承诺中标率或客户可见结论 |

开发调用顺序：

1. 先从 `docs/专题_Stage1-9_缺口收口与优先级清单.md` 确认是 Stage1-2 来源缺口。
2. 再读本节确认来源和路由字段。
3. 需要跨阶段归属时回到 `docs/专题_投前预测_Stage1-9 全链规则与判定资料池.md`。
4. 不直接从 archive 长稿写采集器或规则码。
