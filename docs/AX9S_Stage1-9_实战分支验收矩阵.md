...
| 候选表负责人/证书号解析 | 识别广东评标报告中 `项目负责人姓名及资格证书编号`、`项目负责人` 表格；支持 `公司 + 报价 + 姓名/证书号` 和 `公司 + 报价 + 下浮率 + 姓名` 两类格式 | `primary_responsible_person_name`, `project_manager_certificate_no`, `primary_responsible_person_name_parse_state` | 第一候选人的负责人和证书号可从 PDF 表格补齐；无证书时只补姓名并进入 Stage4 身份补全 | 不得把企业名、评委职称、报价数字、项目编号误当负责人/证书号 | `IMPLEMENTED` | `CODE_CONFIRMED + REPO_RECORDED_PAGE_OBSERVATION` | `real_candidate_capture.py` |
...
- 广东 PDF 表格中 `项目负责人姓名及资格证书编号` 可按 `公司 + 报价 + 姓名/证书号` 补 `primary_responsible_person_name` 和 `project_manager_certificate_no`；只有 `项目负责人` 的表格可按 `公司 + 报价 + 姓名` 补负责人姓名，但证书号保持缺失并进入 Stage4 身份补全。
