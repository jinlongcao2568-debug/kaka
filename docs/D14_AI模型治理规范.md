...
| `M4_PROHIBITED` | 禁用层 | 无 | 一律进入治理复核进入正式链路 |
...
- 经审批允许的治理补证元数据，但正文材料是否可读必须以 D6 / D7 更严格者为准。

### [D14-R-019] 5.2 禁止输入

模型一律不得直接读取或拼接以下输入进入正式链路：

- 未完成审批的 `governed_supplement_record` 正文；
- 未通过字段治理的自然人高限制字段明文；
...
- `EMERGENCY_OFF / PERMANENTLY_BLOCKED` 一旦命中 model/tool/provider family，必须短路生产 live / external mutation path，不得继续进入未授权下游 assist / query / writeback path；sandbox/mock/dry-run/回归测试或明确授权试点按对应 capability mode 和审计要求另行执行。
...
- model/tool provider externalization 仍不得替代 `EXTERNAL_CONTROLLED_OPENING`、human review 或 output target matrix；
...
| 自动退款边界 | 自动退款流程 controlled-test-and-pilot-required；模型不得触发退款，只能辅助 `manual exception` / 人工异常记录、manual approval/audit 和 governed review。 |
