...
| Stage4 释放证据主链 | 62%-65% | 广东/广州施工许可、竣工验收 live canary 已可达可解析；GDCIC 合同履约/项目经理变更正确收口为 `NEEDS_BROWSER`；浙江第一版字段 adapter 已接入并经 canary 验证可区分门户可达、结构化接口阻断、结构化空结果，仍只输出 B/C/D，不抹掉 A 级线索。 |
...
- 完成标准：
  ...
  - 下一步仍需补 GDCIC / 项目经理变更 browser-authorized readback，以及浙江以外具体省份 adapter 的真实字段 readback；不能因为广州住建 API 已通或浙江第一版 adapter 已接入，就误判四类释放证据都已实战稳定
