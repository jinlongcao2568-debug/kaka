...
| Stage1-6 批量实战回归 | 20%-30% | 批量回归账本第一版已补到 `RealPublicStage49PressureReport v1`，会生成 `stage1-6-readiness-table.json` 和 `stage1-6-gap-summary-table.json`，可按候选回答卡在 Stage2/3/4/5/6 哪一站以及下一步动作；但仍未真实跑 20-50 个近期 `07` live 批量，所以实战稳定性仍未完成。 |
