...
- 完成标准：
  ...
  - 下一步仍需补 GDCIC 授权会话后的真实字段命中样本，并用 20-50 个近期 `07` 项目做 Stage1-6 批量实战回归；不能因为广州住建 API 已通、GDCIC 未授权浏览器 canary 已能 BLOCKED、项目经理变更字段抽取已结构化、浙江/四川/江苏/湖北/山东/湖南/河南第一版 adapter 已接入，就误判四类释放证据都已实战稳定
