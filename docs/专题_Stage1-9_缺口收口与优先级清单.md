...
| Stage4 释放证据主链 | 88%-92% | 广东/广州施工许可、竣工验收 live canary 已可达可解析；GDCIC 已有 browser-authorized readback 产物消费和真实浏览器执行器，未授权登录壳实测会落 `BLOCKED`/D，并显式回传授权状态、字段面状态、所需登录态能力，以及 HTTP/Dynamic/Stealthy 不能替代登录态；项目经理变更授权回读已能结构化原项目经理、新项目经理、变更日期、变更原因和责任窗口解释；浙江、四川、江苏、湖北、山东、湖南、河南七个重点省份第一版字段 adapter 已接入；Stage4 real_public pressure 已能把释放证据缺口桥接成 plan-only 字段查询任务，广州 live20 生成 60 条 bridge 任务，dry-run 消费 60 条，live4 canary 得到施工许可/竣工 `NOT_FOUND/D` 与合同/变更 `NEEDS_BROWSER/D`；Stage4 GET/readback 已桥接 Scrapling escalation，POST/cookie/session 路径保持原逻辑。 |
...
| Stage1-6 批量实战回归 | 42%-52% | 已跑广州近期 `07` live20，并修正 readiness 账本分类：5 条 Stage1 复核暂存、1 条 Stage3 负责人角色缺口、14 条 Stage4 公开源/释放证据链缺口；客户可售证据包 0/20。Stage4 主链接入释放证据 bridge 已完成第一版并经 live4 验证；Stage3 负责人误抽已完成第一轮硬化和 live20 无网络回放；下一步把字段查询结果回灌到 Stage6 项目状态，再扩大 20-50 个近期 `07` 批量实战。 |
