...
- Stage1-6 批量回归账本已从“只依赖 Stage1-6 pressure summary”推进到“逐候选逐阶段 readiness 表”：`StageOneSixRealPublicPressureReport v1` 现在会输出 `stage1-6-readiness-table.json` 和 `stage1-6-gap-summary-table.json`，按候选列出 Stage1 候选发现、Stage2 详情抓取、Stage3 字段解析、Stage4 公开核验、Stage5 双闸门、Stage6 事实包状态、当前 bottleneck stage、readiness state 和下一步动作；2026-05-20 已跑广州近期 `07` live30：Stage1 从 88 条原始行中取 30 条有效候选，Stage2 详情快照 30/30，Stage3 parse 30/30，15 条进入 Stage4-6 闭环但全部 `REVIEW_REQUIRED`，15 条只作 Stage1 复核暂存；readiness 分布为 Stage1 复核暂存 15、Stage3 负责人角色缺口 5、Stage4 公开源/释放证据链缺口 10。2026-05-20 已补 Stage3 人名质量门，live20 无网络回放确认“厦门重、质量目标、幢游泳馆、投资、年以上、国电电力、万千瓦、陕西榆林”等非自然人误抽取不再产出，RQSG2 `曾凡伟` 和规划测绘 `胡昌华` 等正样本仍保留；Stage4 字段查询结果已可作为独立输入回灌到 Stage6 多项目状态，live8 产物回放生成 15 个项目状态行，全部正确停在释放证据缺口/来源阻断复核。
- Stage4 real_public pressure 主链已新增释放证据 bridge：`StageOneSixRealPublicPressureReport v1` 现在会输出 `stage4-release-adapter-bridge-table.json` 和可直接喂给 `GuangdongLocalFieldQueryProbe` 的 `stage4-release-adapter-bridge-plan.json`，把 `construction_permit`、`contract_public_info`、`completion_filing`、`project_manager_change_notice` 四类缺口转成 plan-only 字段查询任务；广州 live30 attempt-all 重建生成 120 条任务，施工许可/竣工走广州住建，合同履约 30 条走 GDCIC openplatform 匿名公开 API，项目经理变更 30 条仍走 GDCIC HOME 授权浏览器路径；live3 project-strict canary 实际执行合同履约前 3 条后均为 `NOT_FOUND/D`，且已确认公司其他项目合同不会被误写成本项目 B 级增强证据；2026-05-21 live5 title-variants canary 已继续尝试去除 `初步设计`、`监理`、`工程设计施工总承包`、`施工总价承包招标`、二次招标标记等后缀后的工程主体标题，真实请求已覆盖短标题，但前 5 条仍为 `NOT_FOUND/D`，公司维度公开合同/招投标/参建记录只保留为上下文；同日 projectCode canary 证明 `JG2026-*` 是广州交易中心编号而非 GDCIC 省级项目编码，已拆分为 `tradeProjectCode` 追踪字段，GDCIC `projectCode` 路由只接受明确项目编码字段中的数字省级/工程项目编码；live30 重建后合同任务 30/30 保留 `JG` 追踪号，但 `gdcicProjectCodeVariants` 为 0，字段查询 route plan 不再生成空打的 GDCIC projectCode 路由；v56 已把 bridge 编码召回扩展到 regional readback、source_results、sample_records、source_refs 中的项目编码字段和 URL projectCode 参数，同时屏蔽统一社会信用代码/证书号污染；老 Stage4 GDCIC openplatform 适配器也不再把 `JG2026-*` 放入 `project_codes` 直查，而是保留到 `trade_project_codes` 并回到项目名 lookup；字段查询 summary 已直接输出 `authorization_readiness_state_counts` 和 `operator_next_action_counts`，继续保持“查不到不是无风险、登录态不能被 HTTP/Dynamic/Stealthy 替代”。
...
- 已新增显式全候选 Stage1-6 压测开关：`run-guangzhou-stage1-6-real-public-pressure-v1.ps1 -AttemptAllStage16Candidates`。广州近期 `07` live30 复跑后，30/30 候选全部进入 Stage1-6，原来 15 个候选停在 Stage1 未入选的问题已不再是当前批次卡点。
...
- `run-guangzhou-stage1-6-real-public-pressure-v1.ps1` 已新增 `-EnableAttachmentChallengeResolver`、`-ChallengeTimeoutMs`、`-ChallengeBrowserHeaded`，后续压力流可显式开启广州附件挑战解析，不再靠人工临时设置环境变量。
...
### 2.4 2026-05-21 v55 实战增量

- 广州 live30 合同履约 bridge 的项目代码召回已做一次真实校准：`JG2026-*` 能从候选项目 ID 中稳定抽出，但实测投给 GDCIC `/openplatform/*?projectCode=JG2026-*` 均为 0 行，说明它是交易中心项目编号，不是 GDCIC 省级项目编码。
- 代码已把 `projectCodeVariants`、`tradeProjectCode`、`gdcicProjectCodeVariants` 拆开：`JG` 编号继续进入关键词和人工回溯链，只有 12-22 位数字编码才进入 GDCIC `projectCode` 路由和 code-only 项目匹配。
- live30 bridge 重建确认：合同履约 30 条任务全部保留 `projectCodeVariants=["JG2026-*"]` 与 `tradeProjectCode`，但 `projectCode` 为空、`gdcicProjectCodeVariants` 为 0；plan-only 字段查询确认 30 条合同任务没有生成 `projectCode` 路由。
- 下一步不是扩大 openplatform 盲跑，而是从 `data.ggzy/bid_show`、公告原文、YGP 流程矩阵或已命中的 GDCIC 项目详情中提取数字省级/工程项目编码，再做定向 `projectCode` 查询。

### 2.5 2026-05-21 v56 实战增量

- `StageOneSixRealPublicPressureReport v1` 的 bridge 编码召回已扩展：会从 `regional_hard_defect_source_readback.query_context`、`project_codes/project_code_candidates`、`source_results.query_input`、`sample_records`、`source_refs.stage6.parsed_field_refs` 中明确标注为项目代码/项目编号的字段提取编码，也支持 URL 查询参数中的 `projectCode` 类字段。
- 召回采用字段白名单，不做全文数字抓取；回归样本已覆盖同一 readback 同时出现项目代码和统一社会信用代码时，只采项目代码，不把企业信用代码写入 `projectCodeVariants/gdcicProjectCodeVariants`。
- 老 `GuangdongGdcicOpenplatform` Stage4 适配器已同步修正：`project_code_candidates` 继续保留 `JG2026-*` 血缘，新增 `trade_project_codes`，但 `project_codes` 不再放交易中心编号；JG-only 时先走项目名 lookup，避免未来 Stage4 重新跑时继续空打 `projectCode=JG2026-*`。
- live30 原始产物重建后仍显示 30 条合同任务只有 `JG` 追踪号、无 `gdcicProjectCodeVariants`，说明扩宽召回没有误抽企业信用代码、证书号或 URL 路径数字。
