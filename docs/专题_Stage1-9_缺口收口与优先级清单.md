...
| Stage4 释放证据主链 | 77%-81% | 广东/广州施工许可、竣工验收 live canary 已可达可解析；GDCIC 已有 browser-authorized readback 产物消费和真实浏览器执行器，未授权登录壳实测会落 `BLOCKED`/D，并显式回传授权状态与下一步动作；浙江第一版字段 adapter 已接入并可区分门户可达、结构化接口阻断、结构化空结果；四川第一版字段 adapter 已经真实命中项目列表 + 施工许可详情并输出 B 级增强证据；江苏第一版字段 adapter 已接入结构化 readback，官方入口 live canary 会正确落 `NEEDS_BROWSER/D`，不误判字段命中；Stage4 GET/readback 已桥接 Scrapling escalation，POST/cookie/session 路径保持原逻辑。 |
...
- 完成标准：
  ...
  - 下一步仍需补 GDCIC 授权会话后的字段命中样本、项目经理变更释放证据路径，以及湖北/山东等更多省份 adapter 的真实字段 readback；不能因为广州住建 API 已通、GDCIC 未授权浏览器 canary 已能 BLOCKED、浙江/四川/江苏第一版 adapter 已接入，就误判四类释放证据都已实战稳定
