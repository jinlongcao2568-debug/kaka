...
| Stage1-3 实战稳定性 | 58%-68% | Stage2 已新增 Scrapling parser-only snapshot 增强、单目录 comparison 和多目录 readiness 回放脚本；44 个历史目录中 9 个有 HTML snapshot、33 个 HTML snapshot 被比较，严格附件候选 9/9 稳定；2026-05-20 广州 live20 里 Stage2 详情快照 20/20、Stage3 parse 20/20，说明主链可批量跑；但负责人字段仍有非自然人误抽样本，例如“质量目标、投资、年以上、幢游泳馆、厦门重”，Stage3 人名过滤和字段质量评分仍需硬化。 |
| Stage5 规则门校准 | 50%-60% | A/B/C/D 与“查不到不是没问题”口径已固化；仍缺 20-50 个真实样本误判/漏判校准。 |
| Stage1-6 批量实战回归 | 40%-50% | 已跑广州近期 `07` live20，并修正 readiness 账本分类：5 条 Stage1 复核暂存、1 条 Stage3 负责人角色缺口、14 条 Stage4 公开源/释放证据链缺口；客户可售证据包 0/20。Stage4 主链接入释放证据 bridge 已完成第一版并经 live4 验证；下一步优先硬化 Stage3 负责人误抽，再把字段查询结果回灌到 Stage6 项目状态。 |
