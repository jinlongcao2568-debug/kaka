...
- `RealPublicStage49PressureReport v1` 的 bridge 编码召回已扩展：会从 `regional_hard_defect_source_readback.query_context`、`project_codes/project_code_candidates`、`source_results.query_input`、`sample_records`、`source_refs.stage6.parsed_field_refs` 中明确标注为项目代码/项目编号的字段提取编码，也支持 URL 查询参数中的 `projectCode` 类字段。
