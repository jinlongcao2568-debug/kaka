...
| Stage4 释放证据主链 | 86%-90% | 广东/广州施工许可、竣工验收 live canary 已可达可解析；GDCIC 已有 browser-authorized readback 产物消费和真实浏览器执行器，未授权登录壳实测会落 `BLOCKED`/D，并显式回传授权状态、字段面状态、所需登录态能力，以及 HTTP/Dynamic/Stealthy 不能替代登录态；项目经理变更授权回读已能结构化原项目经理、新项目经理、变更日期、变更原因和责任窗口解释；浙江第一版字段 adapter 已接入并可区分门户可达、结构化接口阻断、结构化空结果；四川第一版字段 adapter 已经真实命中项目列表 + 施工许可详情并输出 B 级增强证据；浙江、四川、江苏、湖北、山东、湖南、河南七个重点省份第一版字段 adapter 已接入；江苏、湖北、山东、湖南、河南官方入口 live canary 会正确落 `NEEDS_BROWSER/D`，不误判字段命中；Stage4 GET/readback 已桥接 Scrapling escalation，POST/cookie/session 路径保持原逻辑。 |
...
| Stage1-6 批量实战回归 | 35%-45% | 已跑广州近期 `07` live20，并修正 readiness 账本分类：5 条 Stage1 复核暂存、1 条 Stage3 负责人角色缺口、14 条 Stage4 公开源/释放证据链缺口；客户可售证据包 0/20。下一步不是继续证明能跑，而是修 Stage4 主链接入释放证据 adapter，并硬化 Stage3 负责人误抽。 |
