...
| 8 | 报价履约与异常低价 | Stage3-6/9 | 后置 | 低价、不平衡报价、付款和履约风险 |
...
| 5 | 正在实施第一刀 | 项目筛选、控标预测、自评分和废标红线先进入内部候选 profile/trace |
| 6 | 待实施主闭环 | 已有 Stage6 内部 trace；公开核验、双闸门和报告收束仍待实施 |
...
| 报价履约与异常低价 | 8 | Stage3-6/9 | 价格和履约风险扩展 | 报价、成本、低价审查记录、付款条款、工程清单、结算资料 | `abnormal_low_price_trigger`、`cost_breakdown_ready`、`low_price_review_record`、付款风险提示 | 后置 |
...
默认下一步推进 **第 5 批主线风险能力层第一刀**：先把项目筛选、控标预测、自评分基础和废标红线做成 Stage3-6 内部候选 profile/trace，不新增正式 D2/D3 对象、规则码、schema 或 migration。
