...
1. 业务证据 SKU：上表这些证据方向，用来决定 Stage1-6 要跑什么来源、抽什么字段、过什么规则。
2. 服务深度档位：`TRIAGE`、`EVIDENCE_PACK`、`DEEP_RELEASE_CHECK`、`CUSTOMER_DELIVERY_READY`，用来决定查到什么深度。
3. `LeadPack SKU-A/B/C`：D8/D10 里的商业封装、交付形态和报价带，不等于业务证据类型，也不等于证据强度。
...
1. 首批能力验证保留五个业务证据 SKU：证书/注册单位/时间异常包、负责人未释放/履约冲突包、信用处罚/监管风险包、综合质疑证据包、投前萝卜标/限制竞争预测包。
