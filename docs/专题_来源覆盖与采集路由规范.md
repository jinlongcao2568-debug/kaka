...
默认核心商业路线是 `POST_CANDIDATE_EVIDENCE_PACK`。当任务涉及候选公示、评标结果、中标结果、开标记录、候选人核查、真实竞争者、控标/围标/串标/陪标证据包或销售承接时，采集应先从后验阶段项目入池，再回溯同一项目全流程材料：

`候选公示/评标结果/中标结果/开标记录 -> 招标公告 -> 招标文件 -> 答疑澄清/补遗 -> 开标记录 -> 投标文件公开 -> 候选公示 -> 中标结果`

`PRE_BID_PREDICTION` 只用于刚发布招标公告、招标文件、答疑澄清、补遗或采购意向阶段。该线可以做是否值得投、控标/定制标预测、废标风险、澄清/质疑建议，但不得输出候选人核查、真实竞争者结论或陪标组合结论。

投前预测还有硬时钟门槛：若同项目已经出现 `05 开标信息`，或者投标截止日期/开标日期已经过去，采集路由不得继续把该项目送入投前预测，只能转入开标后核验、候选后跟踪或证据包路线。若已经出现 `07 中标候选人公示`，直接进入 `POST_CANDIDATE_EVIDENCE_PACK`。
