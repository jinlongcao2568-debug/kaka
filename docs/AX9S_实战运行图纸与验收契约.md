...
flowchart LR
  ...
  S0 --> S1["Stage1 市场扫描<br/>批量过滤, 不单选拟合"]
...
| --- | --- | --- | --- | --- |
| Stage1 市场扫描 | 地区、类型、金额区间、时间窗口、真实候选列表、来源蓝图策略 | `task_record`, `execution_context`, `review_lane_profile`, market scan candidates, source blueprint | 批量过滤所有候选，保留未通过原因；按 source blueprint 选择公开源组合；不能为迎合 owner 固定产出 1 个机会 | `NO_CANDIDATES`, `FILTERED_OUT`, `REVIEW_REQUIRED` |
...
| Stage1 市场扫描 | `src/stage1_tasking/market_scan.py::Stage1MarketScanEngine` | 对候选做地区、金额、公告阶段、截止时间和评分过滤 |
...
| 2. 企业内人员消歧 | 在企业记录下查注册人员/项目负责人；优先用公司 + 项目经理姓名 + 注册证书号/身份证脱敏号/注册专业/等级组合 | 证明这个项目经理属于该候选公司或曾在相关时间属于该公司；唯一匹配时派生证书编号作为后续查询主键 | `manager_personnel_public_record_verification_missing` 或 `manager_identity_ambiguous` |
