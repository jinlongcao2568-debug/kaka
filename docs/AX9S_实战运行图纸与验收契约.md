...
flowchart LR
  ...
  S0 --> S1["Stage1 市场扫描<br/>公示窗口分流, 不单选拟合"]
...
| --- | --- | --- | --- | --- |
| Stage1 市场扫描 | 地区、类型、金额区间、时间窗口、真实候选列表、来源蓝图策略 | `task_record`, `execution_context`, `review_lane_profile`, market scan candidates, source blueprint | 所有候选先入池，真实候选优先保留；中标候选公示/异议窗口是第一层分流；金额、类型、字段完整度只打复核和优先级标签；不能为迎合 owner 固定产出 1 个机会 | `NO_CANDIDATES`, `FILTERED_OUT`, `REVIEW_REQUIRED` |
