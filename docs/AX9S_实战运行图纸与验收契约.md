...
项目负责人未释放或在建履约冲突必须进一步形成官方证据链：

1. 当前项目证据：`07 中标候选人公示`、候选公司/联合体、负责人、证书号、排名、报价、招标文件中的无在建或不得兼任条款。
2. 可能占用项目证据：旧项目中标候选/中标结果、施工许可、合同备案、工程合同信用信息、履约记录、项目经理变更或监管处理公告。
3. 时间窗口：当前项目候选/开标/投标节点与旧项目中标、合同、许可、计划竣工、实际验收、变更时间是否重叠。
4. 释放证据：合同约定工作验收合格或移交、竣工验收/备案、项目经理变更或施工许可变更、非承包方原因停工超过 120 天且建设单位同意、同一工程相邻分段/分期例外。广州项目还要关注建筑施工项目安全生产标准化考评结果告知书。

缺竣工、缺备案、缺项目经理变更或四库/JZSC 未命中，不能直接判定“冲突成立”，也不能反向判定“无在建/无风险”。这些情况只能进入 `STRONG_REPLAYABLE_CLUE`、`MEDIUM_REVIEW_CLUE` 或 `INSUFFICIENT_EVIDENCE`，由 Stage5 双闸门和后续异议/投诉证据包判断证据是否足够。
...
- 不能把“没有竣工证”“四库项目未命中”“旧项目只有候选公示”写成在建冲突成立；只能写未释放风险线索或证据不足。
- 不能把查询未命中、网页阻断或接口失败写成无风险；必须进入公开源 readback 缺口和补源任务。
