...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 负责人未释放/履约冲突包 | 近期 `07` 入池，识别工程 lane、候选组和异议窗口 | 当前项目固定 `07/03/04` 和必要流程；历史在建线索先用 `data.ggzy` 和 `bid_show` 宽筛，`bid_show` 已有负责人 + 合同/交付/履约时间时直接进入时间窗口复核，字段不足或歧义时才用原文地址定向回溯；不全量下载 01-12，08 先登记 | 抽当前候选公司、联合体、负责人、证书、工期、报价、排名；历史项目只抽负责人、公司/联合体、工期/服务期、合同履行期限、中标日期和释放证据线索 | 四库/JZSC、全国公路建设市场监督管理系统、data.ggzy 历史中标、原文 readback、地方许可/合同/竣工/变更 | 时间窗口重叠、释放证据缺口、身份消歧和证据足够性双门；只有同一负责人/公司/联合体成员 + 时间窗口疑似重叠才触发释放证据深查 | 内部未释放风险线索包、宽筛记录、补查清单、证据强度 | 仅在能力成熟后用于买家适配、触达和交付治理 |
...
- 负责人未释放/履约冲突包必须先做宽筛再做深查；负责人未释放/履约重叠宽筛原文回溯只定位含负责人、工期/服务期、合同履行期限、中标日期或释放证据的阶段，不默认全量下载 01-12，也不默认解析 `08`；命中同负责人/同主体/时间窗口重叠先记 `A_STRONG_TIME_OVERLAP_SIGNAL`，后续释放证据只做 B 补强、C 反向解释或 D 证据不足，不抹掉 A 级线索。
- P13B 原文回溯若已抽到候选公司/联合体 + 履约或工期周期，但缺候选负责人姓名，进入 `TARGETED_PERSON_READBACK_REQUIRED`，只对该历史项目做详情页、目标附件和可选 OCR 的定向负责人回溯；定向回溯消费后仍未命中同人时落 `PARK_TARGETED_PERSON_NOT_FOUND`，不得循环生成同一任务，也不得写成排除结论。
...
| 来源蓝图 | 按地区/类型/金额选择省级平台、全国聚合、核验源、信用源、地方住建源 | source blueprint, capture plan | 明确为什么选或跳过每个来源 | 全国聚合不得被当成全量实时源 | `PARTIAL` | `AUTHORITY` | `src/stage1_tasking/source_blueprint.py` |
...
| 负责人未释放/履约重叠宽筛 | 历史项目定向找负责人、工期、释放证据：先用 `data.ggzy` 和 `bid_show`，`bid_show` 已含负责人 + 合同/交付/履约时间时直接形成时间窗口复核；只有字段不足或歧义时，才通过原文链接和对应地区 readback 定位可能含负责人、工期/服务期、合同履行期限、中标日期或释放证据的阶段。释放证据查询优先历史重叠项目所在地住建局/住建主管部门公开源；非广东历史项目按归属地住建局适配器做；A/B/C/D 分级为 A 强重叠线索、B 官方补强、C 反向解释、D 证据不足。 | 不得为了历史重叠宽筛全量下载历史项目 `01-12`，也不得默认解析历史项目 `08`；不得因 D 证据不足而抹掉 A 级线索；不得默认把全国四库、省工程审批系统或广东/广州源作为非广东 B/C/D 主源。 |
...
本表吸收原 Stage1-6 任务契约的高价值硬规则，只保留当前仍有效的执行边界；现行人类主阅读面以本表为准，机器细则保留在 `control/stage16_file_analysis_task_contract.yaml`：
...
本表吸收投前辅助线长稿中最值得产品化的候选，不直接变成规则码，只说明它们在现行 Stage1-6 的位置；跨 Stage1-9 的规则资料入口统一看 `docs/专题_投前预测_Stage1-9 全链规则与判定资料池.md`：
