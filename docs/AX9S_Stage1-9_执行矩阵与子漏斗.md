...
| 负责人未释放/履约重叠宽筛 | 历史项目定向找负责人、工期、释放证据：先用 `data.ggzy` 和 `bid_show`，`bid_show` 已含负责人 + 合同/交付/履约时间时直接形成时间窗口复核；只有字段不足或歧义时，才通过原文链接和对应地区 readback 定位可能含负责人、工期/服务期、合同履行期限、中标日期或释放证据的阶段。释放证据查询优先历史重叠项目所在地住建局/住建主管部门公开源；A/B/C/D 分级为 A 强重叠线索、B 官方补强、C 反向解释、D 证据不足。 | 不得为了历史重叠宽筛全量下载历史项目 `01-12`，也不得默认解析历史项目 `08`；不得因 D 证据不足而抹掉 A 级线索；不得默认把全国四库或省工程审批系统作为 B/C/D 主源。 |
