...
- 辅助产品线：投前预测分析。只用于 `02 招标文件公示`、`03 招标公告/关联公告`、`04 答疑澄清/补遗` 且投标截止/开标未过的项目；标准销售窗口为截止/开标前 168 小时以上，72-168 小时只做限时快筛，少于 72 小时不作为正常投前预测销售；一旦出现 `05 开标信息`，投前预测即不适用，转入开标后核验或候选后证据包路线。
