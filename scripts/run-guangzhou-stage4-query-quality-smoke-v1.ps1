param(
    ...
    [string]$BeforeSourceGapProbeJson = "",
    [string]$OutputRoot = "",
...
if (-not $BeforeSourceGapProbeJson) {
    $BeforeSourceGapProbeJson = Join-Path $repoRoot "tmp\evaluation-real-samples\guangzhou-stage4-source-gap-probe-v1\stage4-source-gap-probe-v1.json"
}
...
$argsList = @(
    "-m", "storage.guangzhou_stage4_query_quality_smoke",
    ...
    "--before-source-gap-probe-json", $BeforeSourceGapProbeJson,
