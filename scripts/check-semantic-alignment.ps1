...
$currentCloseoutToken = 'AUTHORITY[-_ ]CONVERGENCE'
...
$taskIdCarriesCloseoutToken = $false
if ($currentTaskTaskId -and $currentTaskTaskId -match $currentCloseoutToken) {
    $taskIdCarriesCloseoutToken = $true
}
if (-not $taskIdCarriesCloseoutToken -and $currentTaskPacketId) {
    $normalizedTaskId = ($currentTaskTaskId -replace '[_\-\s]+', '').ToUpperInvariant()
    $normalizedPacketId = ($currentTaskPacketId -replace '[_\-\s]+', '').ToUpperInvariant()
    if ($normalizedTaskId.Contains($normalizedPacketId)) {
        $taskIdCarriesCloseoutToken = $true
    }
}
if (-not $taskIdCarriesCloseoutToken) {
    Add-Issue -Bag ([ref]$issues) -Severity 'ERROR' -Code 'CURRENT_TASK_WORKSTREAM_DRIFT' -Message "current_task.yaml task_id must carry $currentCloseoutToken or the active task_packet.packet_id." -Path 'control/current_task.yaml'
}
if (-not $repoWorkstream -or $repoWorkstream -notmatch $currentCloseoutToken) {
    Add-Issue -Bag ([ref]$issues) -Severity 'ERROR' -Code 'REPO_STATUS_WORKSTREAM_DRIFT' -Message "repo_status.md Current Workstream must carry $currentCloseoutToken." -Path 'control/repo_status.md'
}
...
if (-not $referenceIndex) {
    ...
    if (-not ($referenceIndex.PSObject.Properties.Name -contains 'formalSupportDocs')) {
        ...
        $expectedSupportDocs = @{
            statusBoard = '文档与资产状态板.md'
            techDecision = '技术实现决策页.md'
            launchAdjudication = '正式业务代码开发开工裁决页.md'
        }
    ...
    if (-not ($referenceIndex.PSObject.Properties.Name -contains 'navigationAssets')) {
        ...
        if (-not $executionRouteMap) {
            ...
            if ([string]$executionRouteMap.path -ne 'AX9S_开发执行路由图.md') {
                Add-Issue -Bag ([ref]$issues) -Severity 'ERROR' -Code 'REFERENCE_INDEX_NAV_ASSET_DRIFT' -Message 'executionRouteMap.path must equal AX9S_开发执行路由图.md.' -Path 'control/reference_index.json'
            }
