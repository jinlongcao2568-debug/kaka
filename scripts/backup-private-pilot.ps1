...
if ($RequireProductionOverlay -and -not $productionOverlayUsed) {
    throw 'Production backup requires an explicit production Compose overlay.'
}
...
try {
    if ($writerServices.Count -gt 0) {
        docker compose @composeArgs stop --timeout 60 @writerServices
        if ($LASTEXITCODE -ne 0) { throw 'Failed to stop all private-pilot writers.' }
    }

    if ($productionOverlayUsed) {
        docker compose @composeArgs --profile backup pull backup-tools
        if ($LASTEXITCODE -ne 0) { throw 'Failed to pull the immutable production backup-tools image.' }
    }
    else {
        docker compose @composeArgs --profile backup build backup-tools
        if ($LASTEXITCODE -ne 0) { throw 'Failed to build the pinned PostgreSQL 18 backup-tools image.' }
    }

    docker compose @composeArgs --profile backup run --rm backup-tools backup
    if ($LASTEXITCODE -ne 0) { throw 'Private-pilot backup failed closed.' }

    docker compose @composeArgs --profile backup run --rm backup-tools validate
    ...
    if (-not $LeaveWritersStopped -and $writerServices.Count -gt 0) {
        docker compose @composeArgs up -d --no-build @writerServices
...
Write-Output "Validated backup (production_overlay_used=$productionOverlayUsed): $resolvedBackupRoot\$BackupId"
