...
if (-not $SourcePath) {
    $SourcePath = Join-Path $env:LOCALAPPDATA "kaka\internal_operator_loop_store.json"
}
...
$argsList = @(
    "-m", "storage.json_storage_migration",
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "storage json-file to PostgreSQL migration failed with exit code $LASTEXITCODE"
    }
}
finally {
    Pop-Location
