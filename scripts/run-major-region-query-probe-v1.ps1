...
if (-not $OutputRoot) {
    $OutputRoot = Join-Path $repoRoot "tmp\evaluation-real-samples\major-region-query-probe-v1"
}
...
$argsList = @(
    "-m", "storage.major_region_query_probe",
...
if ($RegionCodes) {
    $argsList += "--region-codes"
    $argsList += ($RegionCodes -split "," | ForEach-Object { $_.Trim() } | Where-Object { $_ })
}
if ($SourceProfileIds) {
    $argsList += "--source-profile-ids"
    $argsList += ($SourceProfileIds -split "," | ForEach-Object { $_.Trim() } | Where-Object { $_ })
}
