...
def resolve_source_vendor(
    *,
    ...
    if explicit_vendor_id in registry and str(registry[explicit_vendor_id].get("vendor_role")) == source_role:
        selected_vendor_id = str(explicit_vendor_id)
        ...
        if not selected_vendor_id:
            for vendor_id, entry in registry.items():
                if str(entry.get("vendor_role")) == source_role:
                    ...
                    break
    if not selected_vendor_id:
        selected_vendor_id = f"SRCV-{project_id}"
        resolved_from = "PROJECT_FALLBACK"

    vendor_entry = registry.get(selected_vendor_id, {})
    usage_policy = next(
        (
            entry
            for entry in usage
            if str(entry.get("vendor_role")) == source_role and "7-8" in str(entry.get("stage_range", ""))
        ),
        None,
    )
    fallback_vendor_id = (
        candidate.get("fallback_vendor_id_optional")
        or next((vendor_id for vendor_id in registry if vendor_id != selected_vendor_id and str(registry[vendor_id].get("vendor_role")) == source_role), None)
        or f"NO-FALLBACK-{project_id}"
    )
    return {
        "source_vendor_id_optional": selected_vendor_id,
    ...
    if explicit_vendor_id in registry and explicit_vendor_id in allowed_vendor_ids:
        ...
        for vendor_id in allowed_vendor_ids:
            if vendor_id in registry:
                ...
                break
    if not selected_vendor_id:
        selected_vendor_id = f"EXECV-{project_id}"
        resolved_from = "PROJECT_FALLBACK"
    vendor_entry = registry.get(selected_vendor_id, {})
    execution_policy = next(
        (
            entry
            for entry in execution_policy_entries
            if str(entry.get("vendor_id")) == selected_vendor_id and int(entry.get("stage", 0)) == 8
        ),
        None,
    )
    fallback_vendor_id = (
        candidate.get("execution_fallback_vendor_id_optional")
        or next((vendor_id for vendor_id in allowed_vendor_ids if vendor_id != selected_vendor_id), None)
        or f"NO-FALLBACK-{project_id}"
    )
    return {
        "execution_vendor_id_optional": selected_vendor_id,
