from __future__ import annotations

from typing import Any, Mapping

from stage9_delivery.order_payment_delivery_execution import STAGE9_EXECUTION_LEDGER_OBJECT_TYPE
from storage.db import DatabaseSession, PersistedRecord, build_persisted_at


class Stage9ExecutionLedgerRepository:
    object_type = STAGE9_EXECUTION_LEDGER_OBJECT_TYPE
    id_field = "execution_ledger_id"

    def __init__(self, *, session: DatabaseSession | None = None) -> None:
        self.session = session or DatabaseSession.default()

    def get_by_id(self, record_id: str) -> PersistedRecord | None:
        return self.session.get_record(self.object_type, record_id)

    def find_one_by_field(self, field_name: str, value: str) -> PersistedRecord | None:
        rows = self.session.find_records(self.object_type, **{field_name: value})
        return rows[0] if rows else None

    def get_by_order_execution_id(self, order_execution_id: str) -> PersistedRecord | None:
        return self.find_one_by_field("order_execution_id", order_execution_id)

    def get_by_payment_execution_id(self, payment_execution_id: str) -> PersistedRecord | None:
        return self.find_one_by_field("payment_execution_id", payment_execution_id)

    def get_by_delivery_execution_id(self, delivery_execution_id: str) -> PersistedRecord | None:
        return self.find_one_by_field("delivery_execution_id", delivery_execution_id)

    def save(self, payload: Mapping[str, Any]) -> PersistedRecord:
        payload_dict = dict(payload)
        record_id = str(payload_dict[self.id_field])
        project_id = self._coerce_optional(payload_dict.get("project_id"))
        return self.session.upsert_record(
            PersistedRecord(
                object_type=self.object_type,
                record_id=record_id,
                stage_scope=9,
                project_id=project_id,
                object_refs=self._collect_object_refs(payload_dict),
                decision_states={},
                trace_refs=self._collect_named_values(payload_dict, include_token="trace"),
                audit_refs=self._collect_named_values(payload_dict, include_token="audit"),
                governed_state=self._collect_governed_state(payload_dict),
                writeback_state={
                    "payment_collection_state": payload_dict.get("payment_collection_state"),
                    "delivery_fulfillment_state": payload_dict.get("delivery_fulfillment_state"),
                    "manual_settlement_state": payload_dict.get("manual_settlement_state"),
                    "refund_execution_state": payload_dict.get("refund_execution_state"),
                },
                payload=payload_dict,
                persisted_at=build_persisted_at(),
            )
        )

    def _collect_object_refs(self, payload: Mapping[str, Any]) -> dict[str, str]:
        refs: dict[str, str] = {}
        for key, value in payload.items():
            if key == self.id_field:
                continue
            if key.endswith("_id") or key.endswith("_id_optional"):
                if value not in (None, "", "UNKNOWN"):
                    refs[key] = str(value)
        return refs

    def _collect_named_values(self, payload: Mapping[str, Any], *, include_token: str) -> dict[str, str]:
        refs: dict[str, str] = {}
        for key, value in payload.items():
            if include_token in key.lower() and value not in (None, "", "UNKNOWN"):
                refs[key] = str(value)
        return refs

    def _collect_governed_state(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        return {
            "governed_execution_mode": payload.get("governed_execution_mode"),
            "approval_state": payload.get("approval_state"),
            "audit_state": payload.get("audit_state"),
            "payment_collection_state": payload.get("payment_collection_state"),
            "delivery_fulfillment_state": payload.get("delivery_fulfillment_state"),
            "manual_settlement_state": payload.get("manual_settlement_state"),
            "refund_execution_state": payload.get("refund_execution_state"),
            "automated_refund_enabled": bool(payload.get("automated_refund_enabled", False)),
            "real_payment_gateway_enabled": bool(payload.get("real_payment_gateway_enabled", False)),
            "real_charge_attempted": bool(payload.get("real_charge_attempted", False)),
            "real_delivery_attempted": bool(payload.get("real_delivery_attempted", False)),
            "real_refund_attempted": bool(payload.get("real_refund_attempted", False)),
            "approved_payment_delivery_execution_enabled": bool(
                dict(payload.get("approved_payment_delivery_execution", {})).get(
                    "approved_payment_delivery_execution_enabled",
                    False,
                )
            ),
            "approved_payment_delivery_execution_summary": dict(
                payload.get("approved_payment_delivery_execution", {})
            ),
            "readiness_summary": dict(payload.get("readiness_summary", {})),
        }

    def _coerce_optional(self, value: Any) -> str | None:
        if value in (None, "", "UNKNOWN"):
            return None
        return str(value)


__all__ = ["Stage9ExecutionLedgerRepository"]
