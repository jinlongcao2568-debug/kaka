...
class RuntimeValidator:
    ...
    def _semantic_contact_target(self, payload: Mapping[str, Any], context: Mapping[str, Any]) -> tuple[str, list[str], dict[str, Any]]:
        ...
        if payload.get("auto_contact_allowed"):
            if payload.get("contact_legal_basis") not in ("PUBLIC_ROLE_CONTACT", "CUSTOMER_AUTHORIZED_CONTACT", "COMPLIANT_CRM_CONTACT"):
                decision = "BLOCK"
                reasons.append("auto_contact_allowed requires compliant legal basis")
            if payload.get("opt_out_state") != "ACTIVE" or payload.get("quiet_hours_policy_state") != "ALLOW" or payload.get("frequency_policy_state") != "ALLOW":
                decision = "BLOCK"
    ...
    def _semantic_touch_record(self, payload: Mapping[str, Any], context: Mapping[str, Any]) -> tuple[str, list[str], dict[str, Any]]:
        ...
        if payload.get("response_status") in ("NO_RESPONSE", "DECLINED", "OPTED_OUT", "WRONG_ROLE", "INVALID_CONTACT", "OPPORTUNITY_CHANGED") and not payload.get("written_back_at_optional"):
            decision = "BLOCK"
