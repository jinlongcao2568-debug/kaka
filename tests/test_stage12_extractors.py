...
class TestStage12Extractors(unittest.TestCase):
    ...
    def test_planning_surfaces_keep_transition_safe_active_source_relationship(self) -> None:
        ...
        self.assertIn("专题_Stage1-9_缺口收口与优先级清单", route_map_text)
        self.assertIn("动态推进、当前缺口、缺口减少和优先级", route_map_text)
        self.assertIn("本文件只保留静态导航", route_map_text)
        ...
        controlled_opening_requirement_surface = "\n".join(
            (current_task_text, repo_status_text, route_map_text, json.dumps(active_packet, ensure_ascii=False))
        )
        for controlled_opening_requirement_token in (
            "external release",
            "Stage 8 real execution",
            "Stage 9 real payment / delivery / refund",
            "blocked",
        ):
            self.assertIn(controlled_opening_requirement_token, controlled_opening_requirement_surface)
