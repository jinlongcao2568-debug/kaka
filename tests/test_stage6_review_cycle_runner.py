...
class Stage6ReviewCycleRunnerTests(unittest.TestCase):
    ...
    def test_cycle_bootstrap_merges_supplemental_release_field_query_projection_counts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            loop_runner_test_support._write_blocked_release_field_query_result(root / "field-query")
            ...
            self.assertTrue(result["safe_to_execute"])
            self.assertTrue(
                result["manifest"]["source_supplemental_release_field_query_json"].endswith(
                    "guangdong-local-field-query-probe-v1.json"
                )
            )
            self.assertTrue(
                result["manifest"]["operator_projection_status_table"]["summary"]["continuation_input_refs"][
                    "effective_supplemental_release_field_query_json"
                ].endswith("guangdong-local-field-query-probe-v1.json")
            )
            status_records = {
                record["project_id"]: record
                for record in result["manifest"]["operator_projection_status_table"]["records"]
            ...
            self.assertEqual(projection["release_field_query_task_count"], 2)
            self.assertEqual(
                projection["release_field_query_adapter_result_state_counts"],
                {"NEEDS_BROWSER": 1, "MATCHED": 1},
            )
            self.assertEqual(
                projection["release_field_query_downstream_abcd_grade_counts"],
                {
