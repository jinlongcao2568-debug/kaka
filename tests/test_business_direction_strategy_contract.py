...
class BusinessDirectionStrategyContractTests(unittest.TestCase):
    ...
    def test_p13b_external_award_overlap_triage_policy(self) -> None:
        ...
        self.assertIn("ygp_original_url_readback_from_data_ggzy_pointer", policy["first_pass_sources"])
        self.assertIn("ygp_flow_matrix_01_to_12_node_list_and_detail_readback", policy["first_pass_sources"])
        ...
        self.assertEqual(
            abcd_policy["non_guangdong_history_project_rule"],
        ...
        self.assertTrue(ygp_policy["guangdong_city_adapter_base_capability"])
        self.assertTrue(ygp_policy["active_sample_calibration_disabled_until_city_validated"])
        self.assertIn("YGP_DETAIL_API_DISCOVERED", ygp_policy["readback_states"])
        self.assertIn("YGP_BROWSER_NETWORK_READBACK_READY", ygp_policy["readback_states"])
        ygp_city_policy = policy["ygp_city_discovery_v1_policy"]
        self.assertEqual(ygp_city_policy["source_profile_id"], "GUANGDONG-YGP-CITY-DISCOVERY")
        self.assertEqual(ygp_city_policy["search_endpoint"], "https://ygp.gdzwfw.gov.cn/ggzy-portal/search/v2/items")
        self.assertEqual(ygp_city_policy["default_entry_flow_no"], "07")
        self.assertEqual(ygp_city_policy["default_city_codes"], ["440400", "440500", "440600"])
        self.assertEqual(ygp_city_policy["builds_flow_matrix_with"], "GUANGDONG-YGP-FLOW-MATRIX")
        self.assertTrue(ygp_city_policy["guangdong_other_city_adapter_base_capability"])
        self.assertIn("datasetName", ygp_city_policy["candidate_acceptance_fields"])
        self.assertIn("noticeThirdTypeDesc", ygp_city_policy["candidate_acceptance_fields"])
        self.assertIn("do_not_treat_city_discovery_as_verification_conclusion", ygp_city_policy["must_not"])
        self.assertIn("do_not_default_download_or_parse_flow_08_from_city_discovery", ygp_city_policy["must_not"])
        ygp_matrix_policy = policy["ygp_flow_matrix_v1_policy"]
        self.assertEqual(ygp_matrix_policy["source_profile_id"], "GUANGDONG-YGP-FLOW-MATRIX")
        self.assertTrue(ygp_matrix_policy["not_guangzhou_primary_source"])
        self.assertTrue(ygp_matrix_policy["not_shenzhen_primary_source"])
        self.assertEqual(ygp_matrix_policy["guangzhou_primary_source_remains"], "GUANGZHOU-YWTB-CONSTRUCTION-LIST")
        self.assertTrue(ygp_matrix_policy["shenzhen_primary_source_requires_independent_adapter"])
        self.assertTrue(ygp_matrix_policy["guangdong_other_city_adapter_base_capability"])
        self.assertIn("fetch_trading_notice_new_nodeList", ygp_matrix_policy["readback_flow"])
        self.assertIn("map_node_list_to_01_to_12_flow_matrix", ygp_matrix_policy["readback_flow"])
        self.assertIn("YGP_FLOW_ITEM_NOT_PRESENT", ygp_matrix_policy["flow_item_states"])
        self.assertIn("YGP_DETAIL_READBACK_READY", ygp_matrix_policy["readback_states"])
        self.assertIn("do_not_default_download_or_parse_flow_08_from_ygp_matrix", ygp_matrix_policy["must_not"])
        ygp_full_chain_policy = policy["ygp_full_chain_verification_v1_policy"]
        self.assertEqual(ygp_full_chain_policy["policy_id"], "YGP_FULL_CHAIN_VERIFICATION_V1")
        self.assertEqual(ygp_full_chain_policy["source_profile_id"], "GUANGDONG-YGP-FULL-CHAIN")
        self.assertEqual(ygp_full_chain_policy["default_entry_flow_no"], "07")
        self.assertEqual(ygp_full_chain_policy["default_flow_nos"], ["03", "04", "07", "08"])
        self.assertIn("440200", ygp_full_chain_policy["default_city_codes"])
        self.assertIn("445300", ygp_full_chain_policy["default_city_codes"])
        self.assertIn("ygp_download_probe_for_03_04_07", ygp_full_chain_policy["pipeline_steps"])
        self.assertEqual(ygp_full_chain_policy["download_policy"]["download_default_flow_nos"], ["03", "04", "07"])
        self.assertEqual(ygp_full_chain_policy["download_policy"]["flow_08_policy"], "REGISTER_ONLY_THEN_TARGETED_PARSE_IF_TRIGGERED")
        self.assertIn("ygp-full-chain-manifest.json", ygp_full_chain_policy["output_artifacts"])
        self.assertIn("guangdong-ygp-batch-stability-closeout-v1.json", ygp_full_chain_policy["output_artifacts"])
        self.assertIn("do_not_default_download_or_parse_flow_08", ygp_full_chain_policy["must_not"])
