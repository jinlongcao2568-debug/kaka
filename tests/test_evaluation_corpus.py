...
class TestEvaluationCorpus(unittest.TestCase):
    ...
    def test_probe_classifies_methods_candidates_dark_bright_and_fields(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(len(probes["RANKED"]["candidate_rows_probe_summary"]), 2)
            self.assertEqual(probes["RANKED"]["candidate_rows_probe_summary"][0]["candidate_name"], "甲公司")
            self.assertEqual(probes["RANKED"]["candidate_rows_probe_summary"][0]["project_manager_optional"], "张三")
            self.assertEqual(probes["RANKED"]["candidate_rows_probe_summary"][0]["certificate_no_optional"], "粤144")
    ...
    def test_fairness_markers_are_detected_as_review_signals_only(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("geographic_restriction", probe["fairness_signal_types"])
            self.assertIn("specified_brand_or_supplier", probe["fairness_signal_types"])
            self.assertIn("performance_threshold", probe["fairness_signal_types"])
